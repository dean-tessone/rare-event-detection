...
def generate_tile_paths(path, frame_id, starts, name_format):
    ...
    if len(starts) > 1:
        ...
        paths = [f"{path}/{name_format % (frame_id + starts[0] - 1)}"]
    return paths
...
def filter_events(features, filters, verbosity):
    ...
    for filter in filters:
        ...
        if f_name not in features.columns:
            ...
            sel["index"] = (
                sel["index"] & (features[f_name] >= f_min) & (features[f_name] < f_max)
            )
...
def calc_image_hist(image, bins=2**16, range=[0, 2**16 - 1], density=False):
    ...
    for channel in cv2.split(image):
        hist, _ = np.histogram(channel, bins=bins, range=range, density=density)
...
def calc_bg_intensity(frame, channels_to_mask=["DAPI", "CY5"]):
    ...
    if frame.image is None:
        ...
        for ch in frame.channels:
            props[ch + "_mean_cbg"] = np.mean(frame.image[..., frame.get_ch(ch)][mask])
            props[ch + "_mean_dbg"] = np.mean(frame.image[..., frame.get_ch(ch)][~mask])
...
def obtain_index_tile_from_coord(frame_id, x, y, frames_list):
    idx = frames_list.index(frame_id)
    i = obtain_i(x)
    j = obtain_j(y)
    return idx * 1376 + i * 32 + j


def obtain_tile_idx_from_df_idx(df_idx, df):
    ...
    return int(
        obtain_index_tile_from_coord(
            df["frame_id"][df_idx], df["x"][df_idx], df["y"][df_idx], list_frames
        )
    )
